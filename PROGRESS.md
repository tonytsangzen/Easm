...
1. **HIR / 寄存器分配**（效率阶段主战场）：迭代 15-17 已把 matmul 与
   ...
   **迭代 19（未合入）**：循环暖态两趟编译（pass 1 记录回边缓存状态 →
   截断重编译 → pass 2 头部预填充 + 回边恢复）。暖态循环体寄存器化
   成功，但存在一处未隔离的陈旧值缝隙（sum 余数循环读到相邻局部的
   值）；另修复过"中央 flush 必须位于 insn_at 之后，否则跳向块尾的
   分支会绕过写回"这一关键不变量。时间盒内未收敛，整体回退；下轮
   续作从"重启点重放 flush + 逐局部新旧值断言"入手（EA_WDBG/
   EA_WDBG2 插桩已验证有效）。
