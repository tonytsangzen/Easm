...
1. **HIR / 寄存器分配**（效率阶段主战场）：迭代 15-17 已把 matmul 与
   ...
   **迭代 20 附带修复（已合入，默认路径受益）**：JIT 前序从不零初始化
   ...
   修复后基线上重新验证。
   **EA_WARM 基线重验（零初始化修复后）**：余数循环的 warm 代码经
   逐指令核对为语义正确（槽位映射、值链全对），但 sum(n≤3) 仍错
   ——失败面精确锁定为"主循环被 guard 跳过 + 余数循环运行"的组合；
   错值形态（sum(1)=5 = 第二遍迭代的中间量）指向循环执行了 2 遍
   （计数局部 local3 初值被读为 2 而非 1）。下轮用编译后 pc→机器码
   注释（[A] 插桩，已合入）在 lldb 单步核对运行时寄存器/栈值，
   重点怀疑 guard 路径与余数循环入口的槽位状态衔接。
