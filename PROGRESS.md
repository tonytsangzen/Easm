...
1. **HIR / 寄存器分配**（效率阶段主战场）：迭代 15-17 已把 matmul 与
   ...
   **迭代 20 附带修复（已合入，默认路径受益）**：JIT 前序从不零初始化
   ...
   初始化规范（sum(1) 的余数路径触发；解释器正确）。前序补
   `str xzr` 循环。
   **EA_WARM 运行时值转储（[R] 插桩，已合入）**：循环头处 6 个缓存
   寄存器值打印显示——sum(1) 的余数循环头处 i2/l6/i1/l4 已是循环体
   更新后的值（1/3/6/2）而 s/l5 仍是初值（0/0），且 [R] 仅打印一次
   （head 只到达一次 = 循环实际只跑了一遍）——即**预填充 ldr 从
   frame 装载到的就是"体执行后"的帧值，或 [R] 调用位置与预填充的
   相对顺序和源码顺序不一致**。逐指令静态核对（含正确的
   local_off 映射）显示余数循环发射代码语义完全正确——矛盾指向
   编译期状态与发射代码的对应关系，需 lldb 单步（[A] pc→机器码
   注释已就绪）核对运行时寄存器/栈。
