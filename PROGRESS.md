...
## 迭代 34：垃圾 cur_pc 异常定位记录（2026-09-24，良性留档）

EA_CTRACE 间歇性（~25% 运行，ASLR 相关）打印垃圾 fn_idx/cur_pc
（0x3FFF0000 系 = 疑似 f32 常量位型）。abort 桩 + 前置/后置检查实验：
垃圾打印出现时 abort 不触发（fn_idx 在检查点 ≤1000、在打印点异常），
即损坏发生在 push_x/flush 内部或 CTRACE 读取本身；语义无影响
（三模式 257/258 全绿，含无 CTRACE 运行）。留档待查，不阻塞 HIR-3。
