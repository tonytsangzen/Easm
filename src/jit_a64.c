// easm aarch64 baseline JIT — codegen layer.
//
// Near 1:1 wasm -> AArch64 translation.  This is the upper half of the
// backend: the JC compiler state, the operand-stack primitives, the
// deferred-fusion window, every emitter and the compile driver
// (compile_function / ea_jit_compile_module).  The runtime layer that
// generated code calls into lives in jit_a64_rt.c; the shared layer
// contract is in jit_a64.h.
//
// Machine model (assumed by both layers):
//  - wasm operand stack == native stack, 16-byte slots (SLOT), sp = fp - frame_size;
...
//  - v128 functions fall back to the interpreter transparently.
//
// Operand-stack invariants (the compile-time view the emitters share):
//  - values live as physical slots [sp..), cache refs (cref_local[], value
//    in the local's cache register), the fusion window (def_count/def2/park),
//    or physical FP pushes above the cref layer (n_fpush);
//  - pops drain LIFO: window -> physical FP pushes -> crefs -> stack;
//  - every join (branch target / call / return) flushes to all-physical.
//
// Pointer convention: JC begins with its Em emitter, so &jc->em has the same
// address as jc — emitters freely pass one for the other (the Em*-vs-JC*
// warnings are this idiom, not bugs).
#include "easm.h"
#include "opcodes.h"
#include "a64_emit.h"
#include "jit_a64.h"
...
// ---------------------------------------------------------------- L2a: operand-stack primitives (physical slots / cache refs / FP)
...
// ---------------------------------------------------------------- L2b: deferred-fusion window (parks, in-place, flush)
...
// ---------------------------------------------------------------- L2c: cache-ref layer (cref spill / LIFO consume)
...
// ---------------------------------------------------------------- L4: result emission (push_result, tee fusion, warm bookkeeping)
...
// ---------------------------------------------------------------- L5: exception / call / branch codegen
